...
var (
	...
	race                  bool   = false
	workingDir            string
	serverBinaryName      string = "pseriescollector"
)
...
func makeLatestDistCopies() {
	...
	runError("cp", "dist/pseriescollector_"+version+"_amd64.deb", "dist/pseriescollector_latest_amd64.deb")
	runError("cp", "dist/pseriescollector-"+linuxPackageVersion+rpmIteration+".x86_64.rpm", "dist/pseriescollector-latest-1.x86_64.rpm")
	runError("cp", "dist/pseriescollector-"+version+".linux-x64.tar.gz", "dist/pseriescollector-latest.linux-x64.tar.gz")
}
...
func createDebPackages() {
	createFpmPackage(linuxPackageOptions{
		...
		systemdServiceFilePath: "/usr/lib/systemd/system/pseriescollector.service",

		postinstSrc:    "packaging/deb/control/postinst",
		initdScriptSrc: "packaging/deb/init.d/pseriescollector",
		defaultFileSrc: "packaging/deb/default/pseriescollector",
		systemdFileSrc: "packaging/deb/systemd/pseriescollector.service",
...
func createRpmPackages() {
	createFpmPackage(linuxPackageOptions{
		...
		initdScriptSrc: "packaging/rpm/init.d/pseriescollector",
		defaultFileSrc: "packaging/rpm/sysconfig/pseriescollector",
		systemdFileSrc: "packaging/rpm/systemd/pseriescollector.service",
...
func createMinTar() {
	packageRoot, _ := ioutil.TempDir("", "pseriescollector-linux-pack")
	// create directories
	runPrint("mkdir", "-p", filepath.Join(packageRoot, "/opt/pseriescollector"))
	runPrint("mkdir", "-p", filepath.Join(packageRoot, "/opt/pseriescollector/conf"))
...
func createFpmPackage(options linuxPackageOptions) {
	...
	args := []string{
		...
		"-C", packageRoot,
		"--vendor", "pseriescollector",
		"--url", "http://github.org/adejoux/pSeriesCollector",
		"--license", "Apache 2.0",
...
func ldflags(flags []string) string {
	var b bytes.Buffer
	b.WriteString("-w")
	b.WriteString(fmt.Sprintf(" -X github.com/adejoux/pSeriesCollector/pkg/agent.Version=%s", version))
	b.WriteString(fmt.Sprintf(" -X github.com/adejoux/pSeriesCollector/pkg/agent.Commit=%s", getGitSha()))
	b.WriteString(fmt.Sprintf(" -X github.com/adejoux/pSeriesCollector/pkg/agent.BuildStamp=%d", buildStamp()))
	for _, f := range flags {
...
func clean() {
	...
	rmr("public")
	//rmr("tmp")
	rmr(filepath.Join(os.Getenv("GOPATH"), fmt.Sprintf("pkg/%s_%s/github.com/adejoux/pSeriesCollector", goos, goarch)))
}
